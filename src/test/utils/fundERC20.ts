...
import { Account, PublicClient, TestClient, WalletClient, getContract, parseEther} from 'viem';
import dai from './erc20.json' assert {type:"json"}
import {localAnvil, MNEMONIC} from '../fixtures/fixtures.js'
import { createTestClient, http, createWalletClient, createPublicClient, HttpTransport, Chain, HDAccount} from 'viem'
...
import { setup } from '../fixtures/utils.js';
 ...
 export async function fundDAI(user: string) {
    ...
    if ((await getBalance(walletClient, publicClient, user)) > 1000000000000000000000000n) {
        return;
    }
  ...
  export async function getBalance(walletClient:WalletClient, publicClient:PublicClient, user:string):Promise<bigint> {
