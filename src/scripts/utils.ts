...
// export { deployAndInitializeTokenAndBridgeContracts } from "./cross-chain.js";
import { MNEMONIC, localAnvil } from "./fixtures.js";
//import { isMetricsLoggingRequested, setupMetricsLogger } from "./logging";
...
export const waitForPXE = async (pxe: PXE, logger: any) => {
...
async function setupWithSandbox(
	...
	const cheatCodes = await CheatCodes.create(RPC_URL, pxeClient!);
...
export async function setup(
	...
	if (opts.stateLoad) {
		...
		const ethCheatCodes = new EthCheatCodes(RPC_URL);
	...
	//if (PXE_URL) {
	// we are setting up against the sandbox, l1 contracts are already deployed
	return await setupWithSandbox(hdAccount, "");
	//}
	...
	// 	deployL1ContractsValues.l1ContractAddresses.inboxAddress;
	// config.l1Contracts.outboxAddress =
	// 	deployL1ContractsValues.l1ContractAddresses.outboxAddress;
...
// 	return createDebugLogger("aztec:");
// }
